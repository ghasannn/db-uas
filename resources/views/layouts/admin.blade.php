        ...
        <header class="flex justify-between items-center mb-10 w-full col-span-full">
            ...
            <div class="flex items-center gap-4">
                <div class="text-right hidden md:block">
                    <p class="font-bold">{{ auth('admin')->user()->name ?? 'Pengguna Admin' }}</p>
                    <p class="text-xs text-slate-400">
                        @if(auth('admin')->user() && auth('admin')->user()->isSuperadmin())
                            Superadmin
                        @elseif(auth('admin')->user() && auth('admin')->user()->isOrganizer())
                            {{ auth('admin')->user()->organization->name ?? 'Penyelenggara Event' }}
                        @else
                            Pengguna Admin
                        @endif
                    </p>
                ...
                <div class="w-12 h-12 bg-white rounded-2xl shadow-sm border flex items-center justify-center p-1">
                    <img src="https://ui-avatars.com/api/?name={{ urlencode(auth('admin')->user()->name ?? 'Admin') }}&background=6366f1&color=fff" class="rounded-xl">
                </div>
            </div>
        </header>

        @if(session('success'))
            <div class="bg-emerald-100 text-emerald-800 border border-emerald-200 p-4 rounded-2xl mb-6 font-bold text-sm flex items-center gap-2">
                <span>✅</span> {{ session('success') }}
            </div>
        @endif

        @if(session('warning'))
            <div class="bg-amber-100 text-amber-900 border border-amber-200 p-4 rounded-2xl mb-6 font-bold text-sm flex items-center gap-2">
                <span>⏳</span> {{ session('warning') }}
            </div>
        @endif

        @if(session('error'))
            <div class="bg-rose-100 text-rose-800 border border-rose-200 p-4 rounded-2xl mb-6 font-bold text-sm flex items-center gap-2">
                <span>⚠️</span> {{ session('error') }}
            </div>
        @endif

        @if(session('info'))
            <div class="bg-blue-100 text-blue-800 border border-blue-200 p-4 rounded-2xl mb-6 font-bold text-sm flex items-center gap-2">
