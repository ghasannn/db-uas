    ...
    <form action="{{ route('admin.events.update', $event->id) }}" method="POST" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <input type="datetime-local" name="date" value="{{ old('date', $event->date instanceof \Carbon\Carbon ? $event->date->format('Y-m-d\TH:i') : date('Y-m-d\TH:i', strtotime($event->date))) }}" class="w-full px-5 py-4 bg-slate-50 border-2 border-slate-100 rounded-2xl focus:ring-4 focus:ring-indigo-500/10 focus:border-indigo-600 outline-none transition font-medium" required>
        ...
        <div>
            ...
            @if($event->poster_path)
                <div class="mt-3 p-3 bg-slate-50 rounded-xl border border-slate-100 inline-flex items-center gap-2">
                    <span class="text-xs font-semibold text-slate-500 uppercase tracking-wider">Poster Saat Ini:</span>
                    <a href="{{ asset('storage/' . $event->poster_path) }}" target="_blank" class="text-sm text-indigo-600 hover:underline font-bold inline-flex items-center gap-1">
                        Lihat Foto 
